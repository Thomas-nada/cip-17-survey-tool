...
export function SurveyResponseForm({ survey, onSubmitted }: Props) {
  ...
  const [numericByQuestion, setNumericByQuestion] = useState<Record<string, number>>({});
  ...
  const challengeResponsePayload = useMemo(() => {
  ...
  const toggleOption = (question: SurveyQuestion, index: number) => {
    const questionId = question.questionId;
    if (question.methodType === METHOD_SINGLE_CHOICE) {
      setSelectedByQuestion((prev) => ({ ...prev, [questionId]: [index] }));
    } else {
      setSelectedByQuestion((prevAll) => {
        const prev = prevAll[questionId] ?? [];
        if (prev.includes(index)) {
          return { ...prevAll, [questionId]: prev.filter((i) => i !== index) };
        }
        if (
          question.maxSelections &&
          prev.length >= question.maxSelections
        ) {
          toast.error(t('vote.maxSelectionsAllowed', { count: question.maxSelections ?? 0 }));
          return prevAll;
        }
        return { ...prevAll, [questionId]: [...prev, index] };
      });
    }
  };
      ...
      dispatch({
        ...
        payload: {
          ...
          response: {
            ...
            numericValue: response.answers?.[0]?.numericValue ?? response.numericValue,
            customValue: response.answers?.[0]?.customValue ?? response.customValue,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 animate-fadeIn">
      ...
      {questions.map((question, questionIndex) => {
        ...
        const isCustom = !isOptionBased && !isNumeric;
        const selectedIndices = selectedByQuestion[question.questionId] ?? [];
        const numericValue = numericByQuestion[question.questionId] ?? question.numericConstraints?.minValue ?? 0;
        const customText = customByQuestion[question.questionId] ?? '';

        return (
          <div key={question.questionId} className="space-y-4 bg-slate-800/20 border border-slate-700/30 rounded-xl p-4">
              ...
              {method === METHOD_MULTI_SELECT && (
                <div className="mt-3 flex items-center gap-2">
                  <span className="text-xs font-semibold text-teal-400 bg-teal-500/10 px-2.5 py-1 rounded-lg border border-teal-500/20">
                    {t('vote.selectUpTo', { count: question.maxSelections ?? 0 })}
                  </span>
                </div>
              )}
            </div>

            {isOptionBased && question.options && (
            ...
            {isNumeric && question.numericConstraints && (
              <div className="bg-slate-800/30 border border-slate-700/30 rounded-xl p-6 space-y-5">
                <input
                  ...
                  value={numericValue}
                  onChange={(e) => setNumericByQuestion((prev) => ({
                    ...prev,
                    [question.questionId]: parseInt(e.target.value) || 0,
                  }))}
            ...
            {isCustom && (
              <div className="bg-slate-800/30 border border-slate-700/30 rounded-xl p-6 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="inline-flex rounded-lg border border-slate-700/40 overflow-hidden">
                    <button
                      type="button"
                      onClick={() => setMarkdownPreviewByQuestion((prev) => ({ ...prev, [question.questionId]: false }))}
                      className={`px-3 py-1.5 text-xs font-semibold transition-colors ${
                        markdownPreviewByQuestion[question.questionId]
                    ...
                    <button
                      type="button"
                      onClick={() => setMarkdownPreviewByQuestion((prev) => ({ ...prev, [question.questionId]: true }))}
                      className={`px-3 py-1.5 text-xs font-semibold transition-colors ${
                        markdownPreviewByQuestion[question.questionId]
                          ? 'bg-teal-500/15 text-teal-300'
                          : 'bg-slate-900/40 text-slate-400 hover:text-slate-300'
                ...
                <p className="text-xs text-slate-500">Markdown supported (headings, lists, links, code).</p>
                {markdownPreviewByQuestion[question.questionId] ? (
                  <div className="min-h-[120px] bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3">
                    {customText.trim().length > 0 ? (
                      <MarkdownContent content={customText} />
                    ) : (
                  ...
                  <textarea
                    value={customText}
                    onChange={(e) => setCustomByQuestion((prev) => ({
                      ...prev,
                      [question.questionId]: e.target.value,
                    }))}
                    rows={6}
                    placeholder="Write markdown..."
                    className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 outline-none resize-y font-code text-sm"
                  />
                )}
              </div>
            )}
